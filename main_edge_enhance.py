        ...
        for batch_idx, (batch_s, batch_t) in enumerate(tqdm(
            ...
            if self.cfg.lam_aug > 0:
                ...
                if not self.use_NCE:
                    ...
                    pred_w = self.model(x_w.to(self.device))
                    with torch.no_grad():
                        pred_w_ori= self.model(x_w_ori.to(self.device))
                    #pred_aug_ori= self.model(x_aug_ori.to(self.device))
                    if False:
                        pred_t_WS=self.model.class_t(torch.cat([pred_w[3],pred_aug[3]]).detach())
                    #print('enter pred_t_WS')
                    #exit()
                    #pred_w = self.model(x_w.to(self.device))
                    #pred_aug_1, pred_aug_2, pred_aug_3 = pred_aug[0],pred_aug[1],pred_aug[2]
                    pred_w_1, pred_w_2, pred_w_3 = pred_w[0],pred_w[1],pred_w[2]
                    pred_P_1_w = F.softmax(pred_w_1, dim=1)
                    pred_P_1_w_ori = F.softmax(pred_w_ori[0], dim=1)
                    label_1_w = torch.argmax(pred_P_1_w.detach(), dim=1)
                    label_1_w_ori = torch.argmax(pred_P_1_w_ori.detach(), dim=1)
                    maxpred_1_w, argpred_1_w = torch.max(pred_P_1_w.detach(), dim=1)
                    maxpred_1_w_ori, argpred_1_w_ori = torch.max(pred_P_1_w_ori.detach(), dim=1)
                    #T = self.cfg.pseudolabel_threshold
                    mask_1_w = (maxpred_1_w > T)
                    mask_1_w_f = (maxpred_1_w > self.cfg.Tmax)
                    
                    mask_1_w_ori = (maxpred_1_w_ori > T)
                    mask_1_w_f_ori = (maxpred_1_w_ori > self.cfg.Tmax)
                    
                    mask_1_w_b = ((maxpred_1_w < self.cfg.Tmax).float() * (maxpred_1_w > 0.9299).float())>0
                    #print(mask_1_w_b)
                    
                    #print(mask_1_w_f.sum(),mask_1_w.sum(),(y_aug_1>-1).sum())
                    #print(self.ignore_index,ignore_tensor)
                    #exit()
                    ignore_tensor = torch.ones(1).to(
                        self.device, dtype=torch.long) * self.ignore_index
                    ...
                    label_1_w_ori = torch.where(mask_1_w_ori, label_1_w_ori, ignore_tensor)
                    label_1_w_f_ori = torch.where(mask_1_w_f_ori, argpred_1_w_ori, ignore_tensor)    
                    mask_source_stack=torch.stack(mask_source_list)                    
                    use_erode=True
                    if use_erode:
                        kernel = np.ones((2, 2), np.uint8)
                        mask_1_w_f_numpy=mask_source_stack.cpu().numpy().astype(dtype=np.uint8)
                        erode_mask=[]
                        dilate_mask=[]
                        for i in range(mask_1_w_f_numpy.shape[0]):
                            ...
                            dilate_mask.append(torch.from_numpy((cv2.dilate(mask_1_w_f_numpy[i]*255, kernel)>254).astype(dtype=np.uint8)).reshape(1,mask_1_w_f_numpy.shape[1],mask_1_w_f_numpy.shape[2]))
                               
                        erode_mask=torch.cat(erode_mask)
                        dilate_mask=torch.cat(dilate_mask)
                        bord_mask=(dilate_mask-erode_mask).cuda()
                        if (dilate_mask[0]-erode_mask[0]).sum()==0 or (dilate_mask[1]-erode_mask[1]).sum()==0:
                            use_erode=False
                    #cv2.imwrite('/home/lijj/domain_seg/code/pixmatch-master/pixmatch-master/mask.png',mask_1_w_f_numpy[0]*255)
                    #cv2.imwrite('/home/lijj/domain_seg/code/pixmatch-master/pixmatch-master/mask_enrode.png',(cv2.erode(mask_1_w_f_numpy[0]*255, kernel)>254).astype(dtype=np.uint8)*255)
                    #cv2.erode(mask_1_w_f_numpy[0], kernel)
                    #print(label_1_w_f.shape)
                    #mask_source_stack=torch.stack(mask_source_list)
                    label_source_stack=torch.stack(label_source_list).long()
                    mask_fuse_st=((mask_1_w_f_ori.float()+mask_source_stack)>0).float()
                    #y_aug_1[batch_t_id:batch_t_id+1]=(y_aug_1[batch_t_id:batch_t_id+1]*(1-mask_source_list[batch_t_id].cuda())+label_source_list[batch_t_id]).long()
                    #print(label_1_w_f_ori.shape,label_source_stack.shape,mask_source_stack.shape)
                    #exit()
                    label_fuse_st=label_source_stack*(mask_source_stack)+label_1_w_f_ori*(1-mask_source_stack)
                    label_fuse_st=label_fuse_st.long()                    
                    #exit()
                    if True:
                        #print(self.cfg.Tmax)
                        #print(mask_1_w_f.shape)
                        #print(x_source_aug.shape)
                        #print(y_source_aug_1.shape)
                        #exit()
                        #print(mask_1_w_f_ori.shape,torch.stack(mask_source_list).unique())
                        #exit()

                        #print(label_1_w_f.shape,label_fuse_st.shape)
                        #exit()
                        
                        mask_1_w_f_4=mask_fuse_st.reshape(mask_1_w_f.shape[0],-1,mask_1_w_f.shape[1],mask_1_w_f.shape[2]).float()
                        ...
                        s_t_y=y_source_aug_1*(1-mask_fuse_st.float())+label_fuse_st*mask_fuse_st.float()
                        ...
                        if use_erode_ts:
                            kernel = np.ones((2, 2), np.uint8)
                            mask_1_w_f_numpy=mask_1_w_f_ori.cpu().numpy().astype(dtype=np.uint8)
                            erode_mask_ts=[]
                            dilate_mask_ts=[]
                            for i in range(mask_1_w_f_numpy.shape[0]):
                                erode_mask_ts.append(torch.from_numpy((cv2.erode(mask_1_w_f_numpy[i]*255, kernel)>254).astype(dtype=np.uint8)).reshape(1,mask_1_w_f_numpy.shape[1],mask_1_w_f_numpy.shape[2]))
                                dilate_mask_ts.append(torch.from_numpy((cv2.dilate(mask_1_w_f_numpy[i]*255, kernel)>254).astype(dtype=np.uint8)).reshape(1,mask_1_w_f_numpy.shape[1],mask_1_w_f_numpy.shape[2]))
                                   
                            erode_mask_ts=torch.cat(erode_mask_ts)
                            dilate_mask_ts=torch.cat(dilate_mask_ts)
                            bord_mask_ts=(dilate_mask_ts-erode_mask_ts).cuda()*(1-mask_source_stack)
                            if (bord_mask_ts[0]).sum()==0 or (bord_mask_ts[1]).sum()==0:
                                use_erode_ts=False  
                        if  use_erode_ts:
                        
                            s_t_y_bord=s_t_y*bord_mask_ts-(1-bord_mask_ts)
                            #exit()
                            loss_aug_st = self.loss(pred_s_t_mix_1[:1], s_t_y_bord[:1].long()) * \
                                self.cfg.mix_weight+loss_aug_st                            
                ...
                label_fuse_st_2=label_source_stack*(mask_source_stack)+label_2_w*(1-mask_source_stack)
                label_fuse_st_2=label_fuse_st_2.long() 
                s_t_y_2=y_source_aug_1*(1-mask_fuse_st.float())+label_fuse_st_2*mask_fuse_st.float()    
                loss_aug_st= self.loss(pred_s_t_mix_2[:1], s_t_y_2[:1].long()) * \
                            self.cfg.lam_aug * self.cfg.lam_aux+loss_aug_st       
                if  use_erode_ts and False:
                    s_t_y_bord=s_t_y*bord_mask_ts-(1-bord_mask_ts)
                    #exit()
                    loss_aug_st = self.loss(pred_s_t_mix_1[:1], s_t_y_bord[:1].long()) * \
                        self.cfg.mix_weight+loss_aug_st                   
                if use_erode:
                    #print(bord_mask.shape,bord_mask.unique())
                    y_aug_1_bord=y_aug_1*bord_mask-(1-bord_mask)
                    #exit()
                    loss_aug_1 = self.loss(pred_aug_1, y_aug_1_bord) * \
                        self.cfg.lam_aug+self.loss(pred_aug_1, y_aug_1) * \
                ...
                if self.cfg.aux:
                    if use_erode:
                        label_2_w_bord=label_2_w*bord_mask-(1-bord_mask)
                        loss_aug_2 = self.loss(pred_aug_2, label_2_w_bord) * \
                            self.cfg.lam_aug * self.cfg.lam_aux+self.loss(pred_aug_2, label_2_w) * \
                ...
                if self.use_NCE:
                    ...
                    #print(pred_aug_1.shape,pred_w_1.shape)
                        ...
                        if self.nofalse:
                            ...
                            if n_class_small>1:
                                ...
                                for class_id_temp in class_small:
                                    ...
                                    #print(pred_aug_1_softmax_all[class_ids].shape,pred_aug_1_softmax_all.shape)
                                    #exit()
                        ...
                        y_aug_1=label_fuse_st[batch_id_NCE:batch_id_NCE+1]
