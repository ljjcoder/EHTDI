...
class City_Dataset(data.Dataset):
    def __init__(
        ...
        if split=='train':  
            for i in range(100):
                print(i)
            self.test_ours=False
        #print(self.test_ours)
        #exit()
        if split=='val':
            test_ours=True
            self.test_ours=test_ours
            if self.test_ours:
                self.image_filepath="/home/lijj/domain_seg/code/pixmatch-master/pixmatch-master/test_image/"
    ...
    def __getitem__(self, item):
        ...
        if self.test_ours:
            image_filename=self.files[item]
        ...
        if (self.split == "train" or self.split == "trainval") and self.training:
            ...
            return image, gt_image, image_filename
