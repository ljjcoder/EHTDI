...
def augment(images, labels, aug):
    ...
    aug_images,w_images, aug_labels = [],[], []
    for image, label in zip(images, labels):
        ...
        data = weak(image=image, mask=label)
        image, label = data['image'], data['mask']

        strong_img=aug(image=image, mask=label)
        img_s=strong_img['image']        
        ...
        img_s = torch.from_numpy((cv2.cvtColor(img_s.astype(
            np.float32), cv2.COLOR_RGB2BGR) - IMG_MEAN).transpose(2, 0, 1))         
